...
├── FILE-INDEX.md   # Module → files map — thay thế việc scan codebase
...
Bốn file index **chỉ mô tả và link** — agent đọc index trước, rồi đọc file chi tiết khi cần. Context không bị overload khi project lớn lên.
...
- `planner` — phân tích task, tạo task breakdown (khi không có phase file)
...
- `oryn-dev` — điều phối toàn bộ pipeline, phase-first (bỏ qua planner nếu có phase file)

**On-demand** (`user-invocable: true`):
- `architect` — pre-project system design toàn diện → `system-design.md` + `phase-N.md`
- `phase-writer` — tạo `phase-N.md` theo ưu tiên khi đã biết kiến trúc
...
PostToolUse       → post-edit-audit.sh     # Log mọi file edit vào HISTORY.md
